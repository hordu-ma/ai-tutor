...
from ai_tutor.services.parsing import QuestionParser, TextAnalyzer
...
def test_question_parser():
    ...
    assert len(parsed_questions) > 0 and len(parsed_questions_2) > 0
...
async def test_integration():
    ...
    from ai_tutor.services.student import HomeworkService
    ...
    try:
        ...
        assert len(result.get("parsed_questions", [])) > 0

    except Exception as e:
        logger.error("集成测试失败", error=str(e))
        assert False, f"Integration test failed with {e}"
